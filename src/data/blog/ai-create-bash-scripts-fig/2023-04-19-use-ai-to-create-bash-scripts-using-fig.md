...
![Create a hello world script with Fig.io](./images/1.jpeg)Let's say we have a simple script that echoes "Hello World." We can add this script to Fig and run it by typing: `fig run hello-world`. Fig will then execute the script and output "Hello World."
...
![Generate a snippet using AI and Fig.io](./images/2.jpeg)After we provide GPT with our requirements, GPT comes back with the generated code snippet, title, and description of our script. The generated script is integrated into Fig, along with any user inputs. We can now run this script like any other script in Fig.

![Run a fig script in the terminal](./images/3.jpeg)Fig provides us with a UI for the script, allowing us to input the port number we wish to kill the process at. After providing the input, the script runs, and if a process is running on the given port, it kills that process.
