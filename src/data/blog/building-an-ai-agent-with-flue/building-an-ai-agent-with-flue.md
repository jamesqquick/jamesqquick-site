...
Since joining Cloudflare I've been spending a lot of time with their developer tools, and one thing I wanted to get hands-on with is [Flue](https://flueframework.com) — a TypeScript framework for building AI agents that run on Cloudflare Workers. I'd seen it used in the Cloudflare docs repo and wanted to understand it from the ground up.

So I built something with it: an agent that takes any technical topic and writes a complete developer tutorial. Not a demo — an actual thing I can use. It's a good project because it's simple enough to follow in one sitting but real enough that the Flue concepts actually matter.

This post walks through how I built it. Cloudflare Workers is where the agent runs, but that's almost the last thing we touch. The focus is the agent itself.

## How Flue thinks about agents

Before writing any code, it's worth understanding the four pieces Flue is built around.

A **workflow** is the container — a finite operation that takes input, does work, and returns a result. Every workflow wraps an **agent**: the model configuration. When the workflow runs, it creates a **session** — a conversation thread with the model where the actual work happens.
...
These four pieces — workflow, agent, session, skill — are everything in Flue. Every agent you build is a combination of the same parts.
...
`agents` is Cloudflare's Agents SDK — Flue uses it as the Durable Object foundation on Cloudflare, which is what lets workflows run for minutes without timing out. Normal Workers time out after 30 seconds. Durable Objects can run for hours.
...
The `ai` binding gives your worker access to Workers AI. The `migrations` block is how Cloudflare tracks Durable Object classes — `FlueRegistry` is internal to Flue, and `FlueGenerateWorkflow` maps to `workflows/generate.ts` (naming convention: `Flue` + PascalCase filename + `Workflow`). Every time you add a new workflow in the future, you append a new tagged entry here.

## Defining how the agent thinks

Before giving the agent a specific task, you define its identity — values and operating principles that apply across every session it runs. In Flue, this goes in the `instructions` field on `defineAgent`.

For a tutorial-writing agent, I wanted to push against two failure modes: sounding authoritative while being wrong, and padding content to seem thorough. The instructions I landed on:
...
This lives in the `defineAgent` call, which you'll see in a moment. Notice it says nothing about tutorial structure or output format — those belong in the skill, not the identity.
...
The rules section is the biggest quality lever. Small changes here have more impact than any code change.

## Building the workflow

Create `.flue/workflows/generate.ts`. Start with the skill import — this uses a build-time import attribute that tells the Flue CLI to bundle the markdown file as a skill reference:

```typescript
import writerSkill from '../skills/writer/SKILL.md' with { type: 'skill' };
```

Then define the agent and workflow:
...
`defineAgent` registers the model, instructions, and skills. `defineWorkflow` wraps it with typed input/output schemas and the `run` handler. `harness.session()` creates the conversation thread, and `session.skill('writer', ...)` sends the skill's instructions and args to the model — validating the response against the `result` schema before returning.

The model used here — `cloudflare/@cf/moonshotai/kimi-k2.6` — runs on Workers AI with no API key required.
...
The agent as-is generates from model knowledge alone. The most impactful next step is giving it a `fetch_url` tool backed by Cloudflare Browser Run — fetch a URL, get clean markdown back, let the agent read documentation before writing. For topics involving specific APIs, the quality difference is significant.

After that: an outline phase. A second `session.skill()` call that produces a structured outline before writing starts. The writer session gets the outline as an arg and stays focused on one section at a time instead of planning and writing in a single pass.
